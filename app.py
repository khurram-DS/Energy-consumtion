...
#overall analysis    
 ...
 'Refrigerator - The estimated Watts consumed per day']]
                ...
                st.subheader("Lets compare the different demographic with estimated watts")
                if st.checkbox("Click to compare the estimated watt with different demographics"):
                     all_columns=['الفئة العمرية لمجيب الاستبيان',
 'حجم الأسرة مع العمالة المنزلية',
 'اعلى مؤهل دراسي لرب الأسرة',
 'الحالة الوظيفية',
 'المحافظة تبعا للسكن',
 'إجمالي دخل الأسرة شهريًا بالدينار الكويتي',
 'نوع الوحدة السكنية',]
                     selected_columns= st.selectbox("Select Columns to see Counts and frequency", all_columns)
                     row=df[selected_columns].to_list()
                     selected_row= st.selectbox("Select Columns to see Counts and frequency", row)
                     comp_data=new_data[[selected_columns,
 'Air Conditioner Central Packaged  - The estimated Watts consumed per day',
 'Air Conditioner Central Split  - The estimated Watts consumed per day',
 'Air conditioning Units Mini split  - The estimated Watts consumed per day',
 'Air conditioning Units Windows  - The estimated Watts consumed per day',
 ...
 'Refrigerator - The estimated Watts consumed per day']]
                     ...
                     cmpcolrow=comp_data.loc[comp_data[selected_columns] == selected_row ]
                     cmpcolrow=cmpcolrow.reset_index(drop=True)
                     
                     st.markdown("**All the Data for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     st.write(cmpcolrow)
                     
                     cmpcolrow.columns=cmpcolrow.columns.str.replace('- The estimated Watts consumed per day', ' ')
                     usages_cmpcolrow=pd.DataFrame({})
                     usages_cmpcolrow['Average estimated watts']=cmpcolrow.mean()
                     usages_cmpcolrow['Total estimated watts']=cmpcolrow.sum()
                     usages_cmpcolrow['Minimum estimated watts']=cmpcolrow.min()
                     usages_cmpcolrow['Maximum estimated watts']=cmpcolrow.max()
                     usages_cmpcolrow['Standard Deviation']=cmpcolrow.std()
                     ...
                     st.markdown("**Average and Total Estimated watts for Spatial comfort equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig30 = px.bar(usacom_spat, x="Appliances", y="Total estimated watts",color="Average estimated watts",width=900, height=600)        
                     st.write(fig30)
                     
                     st.markdown("**Minimum And Maximum Estimated watts for Spatial comfort equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig31 = px.bar(usacom_spat, x="Appliances", y="Maximum estimated watts",color="Minimum estimated watts",width=900, height=600)        
                     st.write(fig31)
                     
                     st.markdown("**Overall Estimated watts statistics for Spatial comfort equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     
                     data_comspatial= pd.DataFrame({'minimum': [usages_cmpcolrow['Total estimated watts'].min()],'Mean': [usages_cmpcolrow['Total estimated watts'].mean()],'Maximum': [usages_cmpcolrow['Total estimated watts'].max()]})
                     data_comspatial=data_comspatial.T
                     data_comspatial=data_comspatial.reset_index()
                     data_comspatial.rename( columns={0:'Usages in Watts / day','index':'Statistics'}, inplace=True)
                     
                     fig32 = px.bar(data_comspatial, x="Statistics", y="Usages in Watts / day",width=700, height=500)
                     st.write(fig32)
                     ...
                     usacom_cook = usacom_cook.replace(np.nan, NaN)
                     import plotly.express as px
                     ...
                     st.markdown("**Minimum And Maximum Estimated watts for Cooking equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig34 = px.bar(usacom_cook, x="Appliances", y="Maximum estimated watts",color="Minimum estimated watts",width=900, height=600)        
                     st.write(fig34)
                     
                     st.markdown("**Overall Estimated watts statistics for Cooking equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     
                     data_comscook= pd.DataFrame({'minimum': [usages_cmpcolrow['Total estimated watts'].min()],'Mean': [usages_cmpcolrow['Total estimated watts'].mean()],'Maximum': [usages_cmpcolrow['Total estimated watts'].max()]})
                     data_comscook=data_comscook.T
                     data_comscook=data_comscook.reset_index()
                     data_comscook.rename( columns={0:'Usages in Watts / day','index':'Statistics'}, inplace=True)
                     
                     fig33 = px.bar(data_comscook, x="Statistics", y="Usages in Watts / day",width=700, height=500)
                     st.write(fig33)
                     
                     st.subheader("3. water Equipment")
                     st.markdown("**water equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     st.write(usacom_water)
                     import plotly.express as px
                     st.markdown("**Average and Total Estimated watts for water equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig36 = px.bar(usacom_water, x="Appliances", y="Total estimated watts",color="Average estimated watts",width=900, height=600)        
                     st.write(fig36)
                     
                     st.markdown("**Minimum And Maximum Estimated watts for water equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig37 = px.bar(usacom_water, x="Appliances", y="Maximum estimated watts",color="Minimum estimated watts",width=900, height=600)        
                     st.write(fig37)
                     
                     st.markdown("**Overall Estimated watts statistics for water equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     
                     data_comswater= pd.DataFrame({'minimum': [usages_cmpcolrow['Total estimated watts'].min()],'Mean': [usages_cmpcolrow['Total estimated watts'].mean()],'Maximum': [usages_cmpcolrow['Total estimated watts'].max()]})
                     data_comswater=data_comswater.T
                     data_comswater=data_comswater.reset_index()
                     data_comswater.rename( columns={0:'Usages in Watts / day','index':'Statistics'}, inplace=True)
                     
                     fig38 = px.bar(data_comswater, x="Statistics", y="Usages in Watts / day",width=700, height=500)
                     st.write(fig38)
                     
                     st.subheader("4. Other Appliances Equipment")
                     st.markdown("**Other Appliances equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     st.write(usacom_applian)
                     import plotly.express as px
                     st.markdown("**Average and Total Estimated watts for Other Appliances equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                     fig39 = px.bar(usacom_applian, x="Appliances", y="Total estimated watts",color="Average estimated watts",width=900, height=600)        
                     st.write(fig39)
                     ...
                     st.markdown("**Overall Estimated watts statistics for Other Appliances equipment estimated watts consumed per day for demographic {} and criteria {}**".format(selected_columns,selected_row))
                ...
                st.subheader("Lets check the overall Data without comparison")
